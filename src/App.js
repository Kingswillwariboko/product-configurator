...
function Shoe() {
  ...
  useFrame((state) => {
    ...
    const minDistance = 1; // Set the minimum distance between the camera and the mesh
    const cameraPos = state.camera.position;
    const shoePos = ref.current.position.clone();
    const distance = cameraPos.distanceTo(shoePos);
  
    if (distance < minDistance) {
      // If the mesh is too close, move it away from the camera
      const direction = shoePos.clone().sub(cameraPos).normalize();
      const newShoePos = cameraPos.clone().add(direction.multiplyScalar(minDistance));
      ref.current.position.copy(newShoePos);
    }
  });
